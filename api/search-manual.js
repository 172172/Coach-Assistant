...
<body>
  <div class="wrap">
    ...
    <footer>
      Stabiliserad flöde: GPT anropar <code>search_manual</code> själv → vi svarar enbart med tool_result → GPT formulerar svaret.
    </footer>
  </div>
  ...
  <script>
    ...
    async function connectRealtime(){
      ...
      try{
        ...
        rt.dc.onopen=()=>{
          // Viktigt: skicka INTE om nya instructions/tools här (undvik dubbelkonfiguration).
    ...
    async function hangupRealtime(){
      ...
      rt={ pc:null, dc:null, mic:null, remoteAudio:null, connected:false, calls:new Map() };
    ...
    function handleRealtimeMessage(msg){
      ...
      if (msg.type==='conversation.item.created'){
        if (msg.item?.role==='user'){
          ...
          for (const p of parts){
            ...
            if (typeof text === 'string' && text.trim()) append(bubble(text,'user'));
          }
        }
      ...
      // Samla ihop tool-argument
      if (msg.type==='response.function_call_arguments.delta'){
        const id=msg.id||msg.tool_call_id||msg.response_id||'unknown';
        const prev=rt.calls.get(id)||{name:msg.name,argsText:''};
        ...
        rt.calls.set(id,prev);
        return;
      }

      // Kör verktyget när argumenten är klara
      if (msg.type==='response.function_call_arguments.done'){
        const id=msg.id||msg.tool_call_id||'unknown';
        const tc=rt.calls.get(id)||{name:msg.name,argsText:''};
        let args={}; try{ args=JSON.parse(tc.argsText||'{}'); }catch{ args={}; }
        const shown=(args.query||'').slice(0,48)+(args.query?.length>48?'…':'');
        append(pill(`TOOL CALL → ${tc.name} q="${shown||''}"`));

        if (tc.name==='search_manual'){
          runSearchManualTool(id, args).catch(err=>append(errorBubble('Tool fel: '+(err?.message||String(err)))));
          return;
        }
      }

      // Visa färdiga textsvar
    ...
    async function runSearchManualTool(toolCallId, args) {
      ...
      // LÄGG TILL: Validera och fixa tom query
      if (!args.query || args.query.trim() === '') {
        console.warn('Empty query received from AI');
        // Skicka ett placeholder-svar till AI för att undvika hängande promises
        if (rt.dc && rt.dc.readyState === 'open') {
          rt.dc.send(JSON.stringify({
            ...
            result: JSON.stringify({
              ok: true,
              snippets: [],
              note: "Kunde inte utföra sökning (tom fråga)"
            })
          }));
        }
        return { ok: true, snippets: [], note: "Tom fråga från AI" };
      }
      
      try {
        // 1) Lägg till timeout för API-anropet
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 20000); // 20 sekunder timeout
        
        // 2) Förenkla parametrarna (sänk minSim)
        const params = {
          query: args.query,
          k: typeof args.k === 'number' ? args.k : 5,
          minSim: 0.3,  // Hårdkoda till 0.3 för att säkerställa träffar
          heading: args.heading,
          restrictToHeading: args.restrictToHeading
        };
        
        // 3) Använd AbortController för att kunna avbryta långsamma anrop
        ...
        clearTimeout(timeoutId); // Rensa timeout om anropet lyckas
        
        const data = await response.json();
        console.log('Tool response:', data);
        
        // 4) Returnera alltid något, även om snippets är tomma
        if (!data.snippets || !data.snippets.length) {
          return { ok: true, snippets: [], note: "Inga träffar hittades" };
        }
        return data;
      } catch (error) {
        console.error('Search tool error:', error);
        // 5) Returnera ett komplett svar vid fel för att undvika hängande promises
        return { 
          ok: false, 
          snippets: [], 
