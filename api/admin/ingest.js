...
function splitIntoChunks(md) {
  ...
  for (const line of lines) {
    ...
    if (m) {
      if (cur.content.length) sections.push({ heading: cur.heading, text: cur.content.join("\n").trim() });
...
export default async function handler(req, res) {
  try {
    ...
    if (!markdown || markdown.trim().length < 50) return res.status(400).json({ error: "Markdown saknas eller är för kort" });
