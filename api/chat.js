...
function passesCoverageGate({ coverage, matchedHeadings, scores }) {
  ...
  return coverage >= 0.65 && distinct >= 2 && strongHits >= 2;
}
...
async function createGapDraft({ userId, question, coverage, matchedHeadings, scores }) {
  try {
    ...
    const system = `
Du skapar ett UTKAST när manualen saknar täckning.
- Struktur, inte färdiga fakta. Använd "[PLATS FÖR VÄRDE]" för tal/parametrar.
Returnera JSON: { "title": string, "heading": string, "summary": string, "outline": string[], "md": string }`.trim();
...
export default async function handler(req, res) {
  try {
    ...
    if (isSmalltalk(userText)) {
      ...
      const user = `
Detta är småprat (ingen manual). Text:
"""${userText}"""
Schema:
{"spoken": string, "need": {"clarify": boolean, "question"?: string}, "cards": {"summary": string, "steps": [], "explanation": "", "pitfalls": [], "simple": string, "pro": string, "follow_up": string, "coverage": 0, "matched_headings": []}, "follow_up": string}`.trim();
...
{"spoken": string, "need": {"clarify": boolean, "question"?: string}, "cards": {"summary": string, "steps": string[], "explanation": string, "pitfalls": string[], "simple": string, "pro": string, "follow_up": string, "coverage": number, "matched_headings": string[]}, "follow_up": string}`.trim();
    ...
    // injicera kända fält om modellen glömt
